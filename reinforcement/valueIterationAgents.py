...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for pair in self.mdp.getTransitionStatesAndProbs(state,action):
          value+=pair[1]*(self.mdp.getReward(state,action,pair[0])+self.discount*self.values[pair[0]])
        return value
        ...
        for state in self.mdp.getStates():
          if not self.mdp.isTerminal(state):
            ...
            for action in self.mdp.getPossibleActions(state):
              value = self.computeQValueFromValues(state, action)
              values.append(value)
            pq.update(state, - abs(max(values) - self.values[state]))
        ...
        for i in range(self.iterations):
          ...
          if not self.mdp.isTerminal(temp_state):
            ...
            for action in self.mdp.getPossibleActions(temp_state):
              value = self.computeQValueFromValues(temp_state, action)
              values.append(value)
